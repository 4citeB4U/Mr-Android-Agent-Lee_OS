...
## Overview
Agent Lee now uses **LeeWay-Edge-RTC** as the primary voice and vision transport layer, with **local Qwen model** for intelligence, and **Gemini as a fallback only** for complex tasks.
...
### Before (Gemini-First)
- ❌ Gemini Live WebSocket as primary voice transport
...
- ✅ Gemini only when local capacity insufficient
...
# Gemini API Key (FALLBACK ONLY - optional)
...
### 4. Heavy Lifting (Gemini Fallback Only)
```
...
private async generateResponse(input: string): Promise<string> {
  ...
  try {
    ...
    // Fall back to Gemini or persona rules
...
Only invoke Gemini when Qwen is unavailable:
...
private async generateResponse(input: string): Promise<string> {
  ...
  if (this.isQwenAvailable()) {
    try {
      ...
      console.warn('Qwen failed, falling back to Gemini:', err);
    }
  }

  // 3. FALLBACK: Use Gemini only as last resort
  try {
    return await this.invokeGeminiFallback(input);
  } catch (err) {
...
private async invokeGeminiFallback(input: string): Promise<string> {
  ...
  if (!apiKey) {
    throw new Error('Gemini fallback not configured');
  }

  // Use lightweight Gemini client (not Live API)
...
handleUserSpeech()
    ...
    OR
    → invokeGeminiFallback() → cloud API
    ↓
[RTC State: SPEAKING]
    ↓
speak() → Browser TTS
...
- [ ] **Qwen Offline**: Response still works (uses rules/Gemini)
...
| STT → Response | < 5s | Gemini fallback |
...
- [ ] Check Gemini API key is set (for fallback)
...
4. **Start Agent Lee Voxel OS**: `npm run dev` in agent-lee-voxel-os
...
- Fall back to Gemini only if needed
