...
Agent Lee Voice Core is a **production-oriented, low-latency, real-time voice conversational AI system**. It operates full-duplex, supports barge-in, and is designed to minimise Gemini API usage (targeting the free tier for one primary user + up to ten testers).
...
| `RouterAgent` | Rules + llama.cpp; decides local vs Gemini; never calls Gemini offline |
| `LocalBrainAgent` | llama.cpp GGUF; handles light tasks; streaming tokens |
| `GeminiHeavyBrainAgent` | Gemini API; heavy tasks only; streaming; cancellable |
| `MemoryAgent` | SQLite; stores facts + turns; provides curated context snippets |
| `ProsodyAgent` | Heuristic prosody planning; accepts Gemini tags when online |
...
| Heavy AI | Google Gemini (gemini-1.5-flash, online only) |
