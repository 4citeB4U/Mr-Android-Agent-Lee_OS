...
const fnProxy = path.join(ROOT, 'functions', 'src', 'leewayProxy.ts');
...
if (exists(fnProxy) && exists(fnIndex)) {
  ...
  const hasProxy = /export\s+const\s+leewayProxy\s*=\s*functions\.https\.onRequest/.test(proxyText);
  const hasStream = /export\s+const\s+leewayStream\s*=\s*functions\.https\.onRequest/.test(proxyText);
  const exportsFromIndex = /export\s*\{\s*leewayProxy\s*,\s*leewayStream\s*\}/.test(indexText);
...
// 6) leeway free-tier model policy safety scan
const modelSafetyHits = [];
for (const file of codeFiles) {
  const text = read(file);
  if (/leeway-1\.5-flash|leeway-1\.5-pro/.test(text)) {
