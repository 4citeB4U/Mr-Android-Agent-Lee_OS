...
> Canonical registry of every active agent in the Agent Lee Agentic Operating System as of **April 4, 2026**.
...
| **VisionAgent** | `agents/VisionAgent.ts` | Visual Context Extractor | Screen capture + leeway-vision analysis; emits UI hints and scene summaries | Voice pipeline |
...
| **RouterAgent** | `agents/RouterAgent.ts` | Intent Router & Load Balancer | Classifies each voice turn; decides local LLM vs. leeway routing | Voice pipeline |
...
Mic → AudioCapture (AudioWorklet, 16kHz PCM)
   ...
   → VoicePipeline (asyncio, per-session)
      ...
      ├── RouterAgent (local/leeway decision)
      ├── LocalBrainAgent (llama.cpp GGUF)  ← ~90% of turns
      │   OR leewayHeavyBrain (leeway API)  ← ~10% of turns
...
This document is the canonical registry of the **Living Civilization** within the Agent Lee Agentic Operating System.
