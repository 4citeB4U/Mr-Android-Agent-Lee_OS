...
│  Firebase, leeway, llama.cpp        │     │  8 RTC agents, two-lane architecture     │
└─────────────────────────────────────┘     └──────────────────────────────────────────┘
...
| **Auth** | Firebase Auth (leeway OAuth only) → idToken → Cloud Function proxy |
| **AI / LLM (Local)** | Ollama (E:\ollama-models): gemma4:e2b (reasoning), qwen2.5vl:3b (vision), qwen2.5-coder:1.5b (code/DB); all inference is local except explicit automation |
---
...
| **Functions** | Firebase Cloud Functions (leeway proxy, no key on client) |
...
│  llama.cpp (local) or leeway (cloud) generates response  │
...
- **No raw API key on the client.** leeway calls go through Firebase Functions.
...
| `core/agent_lee_prompt_assembler.ts` | Assembles all modules into a system prompt for leeway |
...
Nine **Core Team** agents handle user-facing work. Each is a TypeScript static class with `respond()`, `generate()`, or `stream()` methods that call `leewayClient` and emit events on the `EventBus`.
...
Six specialist agents manage the complete local-first voice stack. The pipeline runs on device — only complex reasoning turns escalate to leeway.
...
Mic (PCM 16-bit LE 16kHz) → WebSocket → FastAPI server
  ...
  → RouterAgent (local vs. leeway routing decision)
  → llama.cpp (local LLM, ~90% of turns)  OR  leeway 2.0 Flash (cloud ~10%)
...
| **VisionAgent** | `agents/VisionAgent.ts` | VECTOR | Screen & Scene Analyst — screen capture via `getDisplayMedia`, leeway-vision analysis, emits UI hints and scene summary |
| **RouterAgent** | `agents/RouterAgent.ts` | SENTINEL | Intent Router — rule-based fast path (synchronous) + leeway-assisted classification; decides local vs. cloud routing |
...
|---|---|
| **Family** | VECTOR |
| **Purpose** | Screen capture via getDisplayMedia; leeway-vision analysis; emit UI hints and scene summary |
...
|---|---|
| **Family** | SENTINEL |
| **Purpose** | Classify each voice turn intent; decide local llama.cpp vs leeway routing |
| **Fast Path** | Rule-based (sync) → ~90% of turns handled locally |
| **Fallback** | leeway-assisted classification (∼10% cloud escalation) |
| **Status** | 🔵 STANDBY · Ready |
...
**Voice turns** are pre-processed by the voice pipeline before entering G1–G8 routing. The RouterAgent decides at the pipeline level whether to use the local LLM or escalate to leeway.
...
|---|---|---|
| **Z0** — AgentVM | All in-browser execution (React, IndexedDB, EventBus, leewayClient) | Open — high automation |
...
flowchart TB
    ...
    VP --> WS[FastAPI WS Server\nVAD → Whisper → llama.cpp / leeway → Piper]
    WS --> EB
...
│   ├── leewayClient.ts                  leeway REST wrapper (OAuth only, no raw key)
...
│   │       ├── leeway_heavy_brain_agent.py  leeway cloud fallback
...
├── functions/                           Firebase Cloud Functions (leeway proxy)
...
5WH:
  WHAT  = RouterAgent — classifies intent, routes local vs. leeway
  WHY   = Reduces leeway spend; 90%+ of turns handled on-device
  WHO   = Leeway Innovations
  WHERE = agents/RouterAgent.ts
  WHEN  = 2026
  HOW   = Rule fast-path + leeway-assisted classification; emits router:intent

AGENTS: ASSESS AUDIT leeway ROUTER
LICENSE: MIT
*/
```
...
| `VITE_leeway_API_KEY` | leeway API key (dev only — production uses Function proxy) |
| `VITE_VOICE_WS_URL` | Voice WebSocket URL (default: `ws://localhost:8765/ws`) |

> **Security:** `.env.local` is gitignored. Never commit raw API keys. In production, all leeway calls go through the Firebase Functions proxy — no key is exposed to the browser.
