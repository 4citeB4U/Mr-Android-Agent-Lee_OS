...
**Current:** leeway 2.0 Flash Vision
...
| **leeway 2.0 Ultra Vision** | 530B | ⭐⭐⭐⭐ | SOTA | $$$ | Enterprise | API |
...
Replace leeway Vision with Llama 3.2 Vision:
...
Keep leeway as fallback for complex vision tasks:
...
try {
  ...
  console.warn('Local vision failed, using leeway fallback');
  return await leewayVision.analyse(image);
}
```
...
└─ Quality: 95% of leeway 2.0 Flash

vs Current leeway 2.0 Flash:
...
```
leeway Vision: 1000 images × $0.0025 = $2.50
...
VITE_leeway_VISION_FALLBACK=true  # Keep as backup
```
...
### Fallback Strategy
- Keep leeway Vision as backup for complex images
...
4. **Fallback tests**: Verify leeway backup when local models fail
...
- **Fallback**: Keep leeway for complex cases
